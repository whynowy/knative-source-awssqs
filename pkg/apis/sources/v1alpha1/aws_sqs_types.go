...
type AwsSqsSourceSpec struct {
	// Queue URL or ARN of the SQS queue.
	QueueURLOrARN string `json:"queueUrlOrArn"`

	// AwsCredsSecret is the credential used to poll the AWS SQS
	AwsCredsSecret corev1.SecretKeySelector `json:"awsCredsSecret,omitempty"`

	// KIAMOptions is the KIAM config used to poll SQS
	KIAMOptions KiamOptions `json:"kiamOptions,omitempty"`

	// How many Goroutines will be started to listen to SQS
	// +optional
	Workers int `json:"workers,omitempty"`

	// Whethe delivery in bulk
	// +optional
	BulkDelivery bool `json:"bulkDelivery,omitempty"`
...
type KiamOptions struct {
	...
	// SqsIAMRoleARN is the IAM role ARN that can read/write the SQS queue,
	// this role needs to be configured to trust the AssignedIAMRole of Receive Adapter.
	SqsIAMRoleARN string `json:"sqsIamRoleArn"`
}
...
type AwsSqsSourceStatus struct {
	...
	// SinkURI is the current active sink URI that has been configured for the AwsSqsSource.
	// +optional
